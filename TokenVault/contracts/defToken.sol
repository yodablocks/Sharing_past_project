...
contract defToken is Permissions {
    // Using SafeMath to prevent underflow and overflow 
    using SafeMath for uint;

    // Variables
    string  name = "DEF Token"; // SHIB Token for example
    string  symbol = "DEF";
    uint256 public totalSupply; // dynamic supply, the total supply of tokens can change over time
    uint8 decimals = 18;
    uint256 public conversionRate = 100;
    uint256 public totalSupplyHeld;
    
    // balanceOf displays balanceOf DEF Token for an address
    ...
    event Withdrawal(address indexed src, uint _defAmount);
...
// -------------------------------[Deposit & Withdraw]----------------------------------
    function depositETHforDEF() public payable freezeFunction returns(bool success) {
        ...
        emit Swap(msg.sender, "ETH", msg.value, "DEF", msg.value.mul(conversionRate));
        emit TokensMinted(msg.value.mul(conversionRate));
        emit Deposit(msg.sender, msg.value);
        return true;
    }

    function withdrawDEFforETH(uint _defAmount) public freezeFunction returns(bool success) {
        require(balanceOf[msg.sender] != 0, "balance is empty, unable to withdraw");
        require(balanceOf[msg.sender] >= _defAmount, "insufficient funds to withdraw");
        require(_defAmount != 0, "unable to withdraw 0");
        require(_defAmount.div(conversionRate) <= address(this).balance, "contract has no eth available");
        totalSupply = totalSupply.sub(_defAmount);
        totalSupplyHeld = totalSupplyHeld.sub(_defAmount);
        balanceOf[msg.sender] = balanceOf[msg.sender].sub(_defAmount);
        _defAmount = _defAmount.mul(1000000000000000000);
        msg.sender.transfer(_defAmount.div(conversionRate));
        emit Swap(msg.sender, "DEF", _defAmount, "ETH", _defAmount.div(conversionRate));
        emit TokensBurned(_defAmount);
        emit Withdrawal(msg.sender, (_defAmount.div(conversionRate)));
        return true;
    }
}
